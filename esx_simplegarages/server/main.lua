...
ESX.RegisterServerCallback("esx_simplegarages:callback:GetUserVehicles", function(source, cb, garage)
    ...
    local sqlQuery = [[SELECT DISTINCT owned_vehicles.* FROM owned_vehicles, vehicle_keys 
        WHERE 
            vehicle_keys.identifier = @owner AND vehicle_keys.state = "Original" AND owned_vehicles.garage = @garage AND owned_vehicles.type = 'car' AND vehicle_keys.plate = owned_vehicles.plate or
            vehicle_keys.identifier = @owner AND vehicle_keys.state = "Garagenzugriff" AND owned_vehicles.garage = @garage AND owned_vehicles.type = 'car' AND vehicle_keys.plate = owned_vehicles.plate
    ]]

    MySQL.Async.fetchAll(sqlQuery, {['@owner'] = identifier, ['@garage'] = garage, ['@type'] = 'car'}, function(result)
        if result[1] ~= nil then
